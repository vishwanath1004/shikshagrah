...
endPage.controller(
  ...
  function ($scope, $rootScope, $state, $element, $stateParams) {
    ...
    $scope.setCurrentUser = function () {
      if ($scope.isCordova) {
        $scope.currentPlayer = _.has($scope.currentUser, 'handle')
          ? $scope.currentUser.handle
          : $scope.currentUser.name;
      } else {
        $scope.currentPlayer = _.has(globalConfig.context, 'userData')
          ? globalConfig.context.userData.firstName
          : $scope.currentUser.handle;
      }
      $scope.currentPlayerFirstChar = $scope.currentPlayer
        .charAt(0)
        .toUpperCase()
        .slice(0);
    };
    ...
    $scope.checkTemplate = function (contentType) {
      ...
      if (!_.isUndefined(globalConfig.config.endPage)) {
        // check if endpage Manifest/config exist
        var endpageManifest = globalConfig.config.endPage;
        var endpageObj = [];
        if (!Array.isArray(endpageManifest)) {
          // check if it a proper Array of Obj, if not convert
          endpageObj.push(endpageManifest);
          endpageManifest = endpageObj;
        }
        _.each(endpageManifest, function (value, key) {
          // search content type in object and get template
          /* istanbul ignore else */
    ...
    $scope.replayContent = function () {
      ...
      if (
        ...
        content.primaryCategory.toLowerCase() === 'course assessment'
      ) {
    ...
    $scope.handleEndpage = function () {
      ...
      !_.isUndefined($scope.playerMetadata.contentType)
        ? $scope.checkTemplate($scope.playerMetadata.contentType)
        : '';
      $scope.setLicense();
    ...
    $scope.contentLaunch = function (contentType, contentId) {
      ...
      $scope.checkMaxLimit(contentToPlay, function (response) {
        ...
        if (
          ...
          !contentToPlay.content.isCompatible
        ) {
          window.postMessage({
        ...
        if (
          contentToPlay.content &&
          contentToPlay.content.trackableParentInfo
        ) {
        ...
        if (contentToPlay.content.isAvailableLocally) {
          ...
          if (
            ...
            window.parent.hasOwnProperty('onContentNotFound')
          ) {
    ...
    $scope.checkMaxLimit = function (contentToPlay, callback) {
      ...
      if (
        contentToPlay.content &&
        contentToPlay.content.primaryCategory.toLowerCase() ===
          'course assessment'
      ) {
        org.ekstep.service.content
          .checkMaxLimit(contentMetadata)
          .then(function (response) {
            if (response) {
              callback(response);
            } else {
